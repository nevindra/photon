<script setup>
// The `/data` page: a single-header-row shell (AppShell → the global ContextBar) hosting five
// URL-synced tabs — Overview (usage/footprint charts + tiles), Storage (per-signal cards + durable
// strip), Retention (the retention form), Tenants (the federation tenant registry), and Delete
// (per-signal purge actions). The active tab lives in the `?tab=` query param so tabs are
// deep-linkable and survive a refresh. The tab switch folds into the ContextBar's search region as
// a query-driven sub-nav (NavTabItem is a RouterLink), so there's no second header bar; the five
// bodies render conditionally by the active tab. Each body is its own component and owns its data
// composables, mirroring the settings dialog this page retired.
import { computed } from 'vue'
import { useRoute } from 'vue-router'
import { Activity, HardDrive, Timer, Users, Trash2 } from 'lucide-vue-next'
...
import DataTenants from '@/components/data/DataTenants.vue'
...
const TABS = ['overview', 'storage', 'retention', 'tenants', 'delete']
  ...
  <AppShell :mock="api.mock" crumb="Data">
    ...
    <template #toolbar>
      <NavTabs class="text-xs">
        ...
        <NavTabItem
          :to="{ query: { ...route.query, tab: 'tenants' } }"
          :active="tab === 'tenants'"
          data-testid="data-tab-tenants"
        >
          <Users /> Tenants
        </NavTabItem>
        <NavTabItem
    ...
    <section class="flex min-h-0 flex-1 flex-col overflow-y-auto px-5 pt-5 pb-6">
      ...
      <DataTenants v-else-if="tab === 'tenants'" />
