...
// Infrastructure host detail (`/infra/:host`): OS/cores/GPU header + one resource-utilization panel
// per signal (CPU/Memory/Disk/Network/GPU). Sets the global scope to this host on mount so
// "Related ▾" and cross-signal correlation carry `host.name` + the active time window. Mirrors
// RumErrorDetailView's AppShell + `#lead` back-arrow shell and its route-param normalization.
...
import { useInfraHost } from '@/lib/infra/infraQueries'
  ...
  <AppShell :mock="api.mock" :crumb="crumb">
    ...
    <main class="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto p-4">
      ...
      <template v-else>
        ...
        <HostResourcePanels :host="host" :start-ms="startMs" :end-ms="endMs" :has-gpu="hasGpu" />
      </template>
    </main>
  </AppShell>
</template>
