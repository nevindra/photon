...
- **`model.rs`** — domain types: `Channel`/`ChannelInput`, the per-signal `Condition` enum
  (`Metrics`/`Logs`/`Traces`/`Rum`, `#[serde(tag = "signal")]`) each carrying a `Cmp`
  (`gt`/`gte`/`lt`/`lte`) + `threshold` + `window_secs`, `Rule`/`RuleInput`, `Incident`,
  `SeriesSample` (`key: Vec<(String,String)>` + `value: f64`), `AlertPhase`
  (`Ok`/`Pending`/`Triggered`), `SeriesState`, `Transition` (`Triggered`/`Resolved`),
  `SchedulerCommand` (`Upsert(Box<Rule>)`/`Remove(RuleId)`). `Channel`/`ChannelInput` now carry a
  typed **`ChannelConfig`** (`#[serde(tag="type")]`): `Webhook{url,secret,headers}` (HMAC + custom
  headers live only here), `Discord{webhook_url}`, `Telegram{bot_token,chat_id}`. `ChannelKind`
  (`webhook`/`discord`/`telegram`) is derived from the config. `ChannelInput::validate()`
  shape-checks per kind — Discord host-locked to discord.com/discordapp.com/ptb/canary, Telegram
  token `\d+:.+`.
...
- **`format.rs`** — pure `render(channel, &AlertEvent) -> RenderedRequest{url,body,content_type,headers}`
  + `build_generic_payload`; one arm per preset. Generic reproduces the stable JSON body
  (golden-tested) + HMAC; Discord builds a severity-colored embed; Telegram POSTs
  `…/bot<token>/sendMessage` with `parse_mode: HTML`.
- **`notify.rs`** — `Notifier::deliver(&Channel, AlertEvent)` (fire-and-forget, retried ≤3×) +
  `WebhookNotifier::deliver_once` (one awaited POST → real `Result`, for the synchronous test
  route) + `FakeNotifier`.
...
- **`alert_channels`** — `id`, `name` (unique), `kind` (`'webhook'`/`'discord'`/`'telegram'`,
  derived from `config`), `url`, `secret` (nullable — if set, deliveries carry
  `X-Photon-Signature: sha256=<hmac(body)>`), `headers` (nullable JSON object merged into the
  request), `created_at`/`updated_at`. Now also carries a **`config` TEXT** column (per-kind
  `ChannelConfig` JSON) added via the additive `ALTER TABLE … ADD COLUMN` idiom; the legacy
  `url`/`secret`/`headers` columns stay populated (effective endpoint / Generic-only) and a
  `config IS NULL` row (pre-migration) decodes as a `Webhook` config.
...
| `POST /api/alerts/channels/:id/test` | render + send ONE awaited sample delivery to a saved channel; returns `{delivered, error?}` |
| `POST /api/alerts/channels/test` | same, for a DRAFT `ChannelInput` body (create/edit dialog's pre-save Test) |
...
The generic body above is the **Webhook** kind. **Discord** channels receive a native embed (title
`🔴 Triggered · <rule>` / `✅ Resolved · <rule>`, severity color, Value/Threshold/Severity/Series
fields); **Telegram** channels receive an HTML `sendMessage` to
`api.telegram.org/bot<token>/sendMessage`. Rendering is a pure `format::render`;
HMAC/`X-Photon-Signature` + custom headers apply only to the Webhook kind.
...
**Provider presets (shipped):** Discord + Telegram render provider-native messages via `format.rs`;
**Generic Webhook** is unchanged. Slack/Teams/PagerDuty remain booked follow-ups on the same
`kind → formatter` seam. Deep-linking back to Photon awaits a `public_url` config.
...
- **SSRF residual risk.** Channel webhook URLs are user-supplied and the POST is made server-side, so
  ...
  with an egress allowlist / metadata-endpoint block once RBAC / multi-tenancy lands. Discord
  (host-locked) and Telegram (server-constructed `api.telegram.org` endpoint) are **not**
  SSRF-bearing; only the Generic Webhook kind is.
