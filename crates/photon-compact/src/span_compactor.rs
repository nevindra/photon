...
impl<W: Wal> SpanCompactor<W> {
    ...
    pub async fn merge_once(&self) -> Result<usize, PhotonError> {
        ...
        for e in &selected {
            let parquet = Storage::parquet_path_spans(e.segment_id);
            let index = Storage::index_path_spans(e.segment_id);
...
mod tests {
    ...
    #[tokio::test]
    async fn run_once_records_parquet_bytes_in_spans_manifest_entry() {
        ...
        let mut b = SpanBatchBuilder::new(&schema);
        b.append(&span("api", "api", 50));
        b.append(&span("web", "web", 300));
        let wal = Arc::new(FakeWal {
        ...
        let on_disk = std::fs::metadata(tmp.path().join(Storage::parquet_path_spans(SegmentId(0))))
            .unwrap()
            .len();
    ...
    #[tokio::test]
    async fn run_once_drops_empty_span_segment_without_writing_parquet_or_manifest_entry() {
        ...
        let replicator = Arc::new(Replicator::new(storage.clone()));
        let compactor = SpanCompactor::new(wal.clone(), storage.clone(), replicator, schema);
